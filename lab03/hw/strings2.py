...
def verbing(s):
    return s + "ly" if len(s) >= 3 and s.endswith("ing") else s + "ing" if len(s) >= 3 else s
...
def not_bad(s):
    ...
    return s[:not_index] + "good" + s[bad_index+3:] if not_index < bad_index else s
...
def front_back(a, b):
    a_start = a[:len(a)//2] if len(a) % 2 == 0 else a[:(len(a)//2)+1]
    b_start = b[:len(b)//2] if len(b) % 2 == 0 else b[:(len(b)//2)+1]
    a_end = a[len(a)//2:] if len(a) % 2 == 0 else a[(len(a)//2)+1:]
    b_end = b[len(b)//2:] if len(b) % 2 == 0 else b[(len(b)//2)+1:]
